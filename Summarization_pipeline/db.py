...
def upsert_summary(
    ...
    with get_cursor() as cur:
        # Convert embedding list to pgvector format (comma-separated string)
        embedding_str = None
        ...
        if level == 3:
            cur.execute(
                ...
                VALUES (%s, %s, %s, %s, %s::vector(1024), %s, 'done')
                RETURNING id
                """,
                (level, source_id, summary_text, Json(metadata), embedding_str, cluster_doc_ids),
            )
...
def fetch_summaries_with_chinese_text() -> list[dict]:
    """
    استعادة جميع الملخصات التي تحتوي على نصوص صينية
    """
    with get_cursor() as cur:
        cur.execute(
            f"""
            SELECT id, level, source_id, summary_text, metadata
            FROM {config.table_summaries}
            WHERE status = 'done'
              AND (summary_text ~ '[\u4E00-\u9FFF]'
                   OR summary_text ~ '[\u3040-\u309F]'
                   OR summary_text ~ '[\u30A0-\u30FF]')
            ORDER BY level, id
            """
...
def update_summary_text(summary_id: int, new_text: str) -> None:
    """
    تحديث نص الملخص
    """
    with get_cursor() as cur:
        ...
        logger.info(f"Updated summary {summary_id} with cleaned text.")
